...
from src.user_interface.restapi.dto.summary import OrderBy
...
class SummaryQuery:
    ...
    async def get_list(
        ...
        subq = (
            select(SummaryDao.url, func.max(SummaryDao.id).label("max_id"))
            .group_by(SummaryDao.url)
            .subquery()
        )
        ...
        query = (
            base_query.order_by(asc(order_column) if _asc else desc(order_column))
            .offset(offset)
            .limit(limit)
        )
        result_set = await session.scalars(query)
        summaries = [
            SummaryMapper.dao_to_entity(dao).model_dump() for dao in result_set
        ]
